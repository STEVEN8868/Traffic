...
def Data_load(timesteps_input, timesteps_output):
    ...
    NATree = pd.read_csv("./data_set/SmallScaleAggregation/distance_50.csv", header=None).head(8640).to_numpy(np.float32)
        #np.load("./data_set/SmallScaleAggregation/adjmat_50.csv").astype(np.float32)
